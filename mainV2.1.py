...
newStops = pd.DataFrame({'Location':['347 Georges RD Dayton NJ 8810', '614 Cranbury RD East Brunswick NJ 8816', '1601 Perrineville RD Monzoe 08831', '651 Ridge RD Monomouth Junction 8852', 'United States Postal Service, 65 N Main St, Cranbury, NJ 08512']})
...
for newStop in newStops['Location']: #for the newstop
          ...
          def InsertIntoLocationsDf(locations,newStop, closest_location_Index):
                ...
                if 'e' in locals():
                  ...
                  duration2 = distanceAB + distanceBC + distanceCD 
                else: 
...
for location in newRoute['Location']:
  while currentLocationCount < amountOfLocations:
      ...
      def findDistance(location, nextLocation):
        ...
        distanceValueColumn.append(f'{round(distance/1609.34,1)} miles') #to convert from meters to miles
      ...
      findDistance(location, nextLocation)
      currentLocationCount += 1;

newRoute['Distance (Text)'] = distanceTextColumn
newRoute['Duration (Text)'] = durationTextColumn
newRoute['Distance (miles)'] = distanceValueColumn
newRoute['Duration (minutes)'] = durationValueColumn
newRoute.to_csv("newRoute.csv")
